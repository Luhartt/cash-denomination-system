...
class TransactionHistory:
    """
    Create a file named transactions.csv
    Change it to what you see fit
    """
    ...
    def add_transaction(self, amount):
        # If file is empty (Default Values)
        current_balance = 0
        ...
        if os.path.exists(self.filename):
            with open(self.filename, 'r') as file:
                ...
                for row in reader:
                    if row:
                        # Updated Values
                        transaction_number = int(row[0]) + 1 # Take last transaction number and add 1
                        current_balance = float(row[3])      # Take the last balance
        ...
        if choice == '1':
            ...
            if not transactions:
                ...
                for t in transactions:
                    print(
                        f"{t['transaction_number']:<5}|  {t['date']:<10}|  ${t['amount']:>9.2f}|  ${t['balance']:>8.2f}")
